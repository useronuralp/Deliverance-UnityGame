...
public class BlockHitStateMachine : StateMachineBehaviour
{
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        sm_CurrentRecievedAttack = sm_Movementscript.m_LockTarget.GetComponent<CombatBehaviour>().m_CurrentAttack; //Set the name of the attack which the character is recieving in this state.

        //Depending on where the attack comes from, the character will get knocked back to the opposite of that attack.
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (sm_KnockBackTimer >= 0.0f) //Move the character.
        ...
        if (sm_HealthStaminaScript.m_CurrentStamina <= 0) //During guarding, if the your stamina runs out, the character will get stunned. 
        {
            ...
            sm_CombatScript.m_Animator.SetTrigger("GetStunned"); //Exit this state by setting this trigger.
